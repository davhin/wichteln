...
st.write(
    "Do a secret santa style gift exchange with a group of friends, family, or colleagues!"
)
...
recipient_code = st.text_input(
    "Get the name of the lucky person who receives a gift from you!"
)
if recipient_code == "":
...
st.write(
    "For each person, select who they will not receive a gift from"
)

for name in names:
    col1, col2 = st.columns([1, 3])
    col1.write(name)
    names_constraints[name] = col2.multiselect(
        label="excl",
...
names_pseudonyms = dict(
    zip(names, ut.get_n_random_identifiers(len(names)))
)
print(names_pseudonyms)
ut.insert_solution_in_db(names_pseudonyms)
solution = {
    name: names_pseudonyms[solution[name]] for name in solution.keys()
}
st.json(solution)
